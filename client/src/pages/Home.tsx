...
export default function Home() {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/xvgoqjdp', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      if (response.ok) {
        toast.success("¡Mensaje enviado! Nos pondremos en contacto contigo en menos de 24 horas.");
        form.reset();
      } else {
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <select 
                    ...
                    required
                    disabled={isSubmitting}
                    className="mt-2 w-full rounded-2xl border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
